    ...
    for i in output_range:
        ...
        for train, test in kfold.split(X, y[:, index_of_y_to_classify]):
            ...
            if len(np.unique(y_train)) == 1:
                print(f"Skipping fold {counter} due to only one class in the training set.")
                counter += 1
                continue

            # Print class distribution in the training set for the current fold
            print(f"Fold {counter} - Training Set Class Distribution:")
            print(pd.Series(y_train).value_counts())

            # Print class distribution in the testing set for the current fold
            print(f"Fold {counter} - Testing Set Class Distribution:")
            ...
            # 1- linear regression
            linear_classifier = LogisticRegression(random_state = random_state, max_iter=1000, class_weight='balanced')
